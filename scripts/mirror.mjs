...
const replacements = new Map([
  ...
  [
    "\u00a9 2025 ULTIMATE HORIZONS PROPERTY",
    "\u00a9 2025 ULTIMATE HORIZONS PROPERTY" + poweredByDigitalMovement,
  ],
]);
...
/* Keep all page content painted while scrolling. The inherited Elementor motion
   system hid widget contents before repainting them, exposing card outlines. */
body.home.page-id-4371 .elementor-motion-effects-element,
body.home.page-id-4371 .elementor-motion-effects-layer,
body.home.page-id-4371 .azura-reveal,
body.home.page-id-4371 .elementor-invisible {
  opacity: 1 !important;
  visibility: visible !important;
  transform: none !important;
...
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('.elementor-invisible').forEach(function (node) {
    node.classList.remove('elementor-invisible');
  });
  document.querySelectorAll('[data-settings*="motion_fx_"]').forEach(function (node) {
    try {
      var settings = JSON.parse(node.getAttribute('data-settings') || '{}');
      Object.keys(settings).forEach(function (key) {
        if (key.indexOf('motion_fx_') === 0) delete settings[key];
      });
